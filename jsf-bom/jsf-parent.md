...
`io.soil:jsf-parent:0.0.1` (packaging=pom)

JSF 框架的父 POM 模块，为业务项目提供统一的构建配置和多环境 Profile 管理。

> **继承链**：业务项目 → `jsf-parent` → `jsf-dependencies`（聚合 BOM）
>
> 继承 `jsf-parent` 即自动获得 `jsf-dependencies` 的全部依赖版本管理与插件配置。
...
| `d-live` | live | `crpi-gq7wp4xitzhccl7a.cn-guangzhou.personal.cr.aliyuncs.com` | 线上环境 |
