...
| jsf-common | jsf-common | 通用工具类和基础 POJO，提供集合、日期、JSON、JDBC 分页等工具 |
---
...
#### io.soil.waf — PID 文件写入

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `AppPidFileWriter` | `io.soil.waf` | 应用 PID 文件写入器，将进程 ID 写入 app.pid | [AppPidFileWriter](io.soil.waf/AppPidFileWriter.md) |

#### io.soil.waf.config — 配置类

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `WafConfig` | `io.soil.waf.config` | WAF 配置类，配置 Jackson 序列化/反序列化规则和组件扫描 | [WafConfig](io.soil.waf.config/WafConfig.md) |

#### io.soil.waf.controller — 控制器

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `ProbeController` | `io.soil.waf.controller` | 健康探针控制器，提供应用活跃状态检测接口 | [ProbeController](io.soil.waf.controller/ProbeController.md) |

#### io.soil.waf.exception — 异常处理

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `WafException` | `io.soil.waf.exception` | WAF 异常类，用于 Web 应用框架层的异常处理 | [WafException](io.soil.waf.exception/WafException.md) |
| `WafHttpExceptionResponse` | `io.soil.waf.exception` | WAF 异常响应内容，用于封装异常信息返回给客户端 | [WafHttpExceptionResponse](io.soil.waf.exception/WafHttpExceptionResponse.md) |
| `WafHttpExceptionResolver` | `io.soil.waf.exception` | WAF 全局异常处理器，自动捕获并处理 Web 层异常 | [WafHttpExceptionResolver](io.soil.waf.exception/WafHttpExceptionResolver.md) |

#### io.soil.waf.handler — 序列化处理器

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `BigNumSerializer` | `io.soil.waf.handler` | 超出 JS 最大最小值处理，序列化数字为字符串防止精度丢失 | [BigNumSerializer](io.soil.waf.handler/BigNumSerializer.md) |

#### io.soil.waf.util — 工具类

| Class | Package | Description | Doc |
|-------|---------|-------------|-----|
| `RemoteAddrUtil` | `io.soil.waf.util` | 远程地址工具类，用于获取客户端真实 IP 地址 | [RemoteAddrUtil](io.soil.waf.util/RemoteAddrUtil.md) |
